...
class AmecoApp {
    ...
    checkForHealthIssues(formData) {
        ...
        ['q12', 'q13', 'q14'].forEach(key => {
            console.log(`- Checking conditions.${key}:`, formData.conditions?.[key]);
            if (formData.conditions && formData.conditions[key] === 'si') {
                details.push(conditionsQuestions[key] || `Condición ${key}`);
                console.log(`  ✓ Found issue in conditions.${key}`);
            }
        });
        ...
        for (let i = 1; i <= 8; i++) {
            const key = `f${i}`;
            console.log(`- Checking fatigue.${key}:`, formData.fatigue?.[key]);
            if (formData.fatigue && formData.fatigue[key] === 'si') {
                details.push(fatigueQuestions[key] || `Fatiga ${key}`);
                console.log(`  ✓ Found issue in fatigue.${key}`);
            }
        }
