...
DECLARE
    ...
    user_record RECORD;
BEGIN
    -- Insertar o actualizar firmas para todos los usuarios del Turno B
    FOR user_record IN 
        SELECT id, name FROM users WHERE shift = 'B'
    LOOP
        -- Eliminar firma existente si hay
        DELETE FROM digital_signatures WHERE user_id = user_record.id;
...
BEGIN
    ...
    LOOP
        ...
        IF shift_record.id IS NOT NULL THEN
            ...
            WHERE shift_id = shift_record.id AND day_number = 6;
